    ...
    if (customer) {
      if (removeAll) {
        ...
        customer.cart = customer.cart.filter(
          (cartItemId) => cartItemId.toString() !== productId.toString()
        );
      } else {
      ...
      res.status(200).json({
        message: `Product ${
          removeAll ? "removed entirely" : "removed"
...
export const getPurchasedProduct = catchAsyncError(async (req, res, next) => {
  const { id } = req.params;
