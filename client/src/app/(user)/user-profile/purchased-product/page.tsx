...
const Page: React.FC = () => {
  ...
  return (
   <div className="w-full rounded-lg overflow-hidden bg-white px-6 py-4">
      <h1 className="text-xl font-semibold mb-6 text-gray-800">Purchased Products</h1>
      ...
      <div className="flex">
        ...
        />
        <div onClick={fetchProducts} className="cursor-pointer bg-webred px-4 py-2 flex justify-center items-center text-white rounded-l-none rounded-r-md text-lg"><IoSearchOutline/></div>
        </div>
       ...
       {loading?(
        ...
        <table className="w-full border-collapse">
          <thead className="bg-[#F1F1F1] text-black">
            <tr>
              <th className="px-6 py-3 border-b text-sm font-medium tracking-wider">Media Type</th>
              <th className="px-6 py-3 border-b text-sm font-medium tracking-wider">Name</th>
              <th className="px-6 py-3 border-b text-sm font-medium tracking-wider">Categroy</th>
              <th className="px-6 py-3 border-b text-sm font-medium tracking-wider">Amount</th>
              <th className="px-6 py-3 border-b text-sm font-medium tracking-wider">ProductId</th>
              <th className="px-6 py-3 border-b text-center text-sm font-medium tracking-wider">Actions</th>
            </tr>
          </thead>
          <tbody className="bg-white">
            
            {purchasedProducts && purchasedProducts.length > 0 && purchasedProducts?.map((purchasedProduct) => (
              <tr
  ...
  <div className="relative w-40 h-20">
    ...
    <div className="w-full h-full rounded-lg overflow-hidden">
      ...
      />
                ...
                <td className="px-6 py-4 text-gray-700">{purchasedProduct.product.title}</td>
                <td className="px-6 py-4 text-gray-700">{purchasedProduct.product.category?.map((category) => category).join(", ")}</td>
                <td className="px-6 py-4 text-gray-600"><div className="flex justify-center gap-1 items-center"><span><FaRupeeSign/></span><span>{purchasedProduct.product.variants[0].price}</span></div></td>
                <td className="px-6 py-4 text-gray-600">{purchasedProduct.product.uuid}</td>
                <td className="px-6 py-4 " >
                  ...
                  <button className="px-2 py-1 border-1 border-[#22C55E] rounded-lg text-[#22C55E] hover:text-white hover:bg-[#22C55E]">
                   ...
                   <Link href={`/${purchasedProduct.product.mediaType}/${purchasedProduct.product.uuid}`} className="flex gap-2 justify-start items-center">
                      <span>View</span>
      ...
      {totalPages > 0 && (
        ...
        <div className="text-[#999999] w-1/3 ">Showing {((currentPage-1)*dataPerPage)+1} to {((currentPage-1)*dataPerPage+dataPerPage)>totalPurchased?totalPurchased:((currentPage-1)*dataPerPage+dataPerPage)}  of {totalPurchased} Entries </div>
