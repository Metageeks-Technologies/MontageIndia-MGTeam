...
const Form4 = ({ formData }: any) => {
  ...
  const handleSave = async (field: string) => {
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        const updatedDataResponse = await instance.get(
          `/product/${initialData.uuid}`
        );
  ...
  const handleSaveVariant = async (index: number) => {
    ...
    if (
      ...
      const allVariantsValid = data.variants.every(
        (variant) =>
          variant.label?.trim() !== "" &&
          variant.price > 0 &&
          variant.credit > 0
    ...
    try {
      ...
      const isAllVariantsValid = data.variants.every(
        (variant) => variant.label && variant.price
      );
      setIsPublishButtonDisabled(isAllVariantsValid);
    } catch (error: any) {
      const isAllVariantsValid = data.variants.every(
        (variant) => variant.label && variant.price && variant.credit
      );
      setIsPublishButtonDisabled(isAllVariantsValid);
      ...
      case "audio":
        return (
          <>
            <audio controls>
              <source
                src={`https://${BucketName}.s3.${AwsRegiosn}.amazonaws.com/${data.thumbnailKey}`}
                type="audio/mpeg"
              />
            </audio>
          </>
  ...
  useEffect(() => {
    getCategories();
    const allVariantsValid = data.variants.every(
      (variant) => variant.label?.trim() !== "" && variant.price > 0
    );
    setIsPublishButtonDisabled(allVariantsValid);
  }, []);
      ...
      {loading ? (
        ...
        <div className="p-6 mx-auto">
          ...
          <form
            ...
            className="bg-pureWhite shadow-lg rounded-lg overflow-hidden"
          >
            <div className="p-6 ">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="flex justify-between items-center mb-3">
                      ...
                      <div className="flex space-x-2">
                        <button
                          type="button"
                          className={`text-xl ${
                            !editMode.description ? "hidden" : "block"
                          }`}
                          onClick={() => {
                ...
                <div className="mb-6">
                  ...
                  <div className="border-dashed border-2 flex justify-center items-center border-gray-300 p-4 mx-auto rounded-lg mb-5">
                    {renderMedia()}
                  </div>
                  {data.mediaType === "audio" &&
                    data.variants.map((variant, index) => (
                      <div
                        key={variant._id}
                        className="bg-pageBg-light p-4 rounded-lg"
                      >
                        <div className="flex justify-between items-center mb-2">
                          ...
                          {editingVariantIndex === index ? (
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                handleEditToggle("variants", index)
                              }
                            >
                              <FaRegEdit size={20} />
                            </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                {data.mediaType !== "audio" &&
                  data.variants.map((variant, index) => (
                    <div
                      key={variant._id}
                      className="bg-pageBg-light p-4 rounded-lg shadow-sm"
                    >
                      <div className="flex justify-between items-center mb-4">
                        ...
                        {editingVariantIndex === index ? (
                          ...
                          <button
                            type="button"
                            onClick={() => handleEditToggle("variants", index)}
                            className="text-gray-600 hover:text-gray-700 transition-colors"
                          >
                            <FaRegEdit size={24} />
                          </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                      ...
                      {data.tags.map(
                        (tag, index) =>
                          tag.trim() !== "" && (
                            <div
                              key={index}
                              className="flex items-center gap-2 bg-pureWhite-light border border-gray-300 rounded-lg px-3 py-1 shadow-sm"
                            >
                              {editMode.tags ? (
                                <>
                                  <input
                      ...
                      <div className="flex flex-wrap gap-2">
                        {data.category
                          ...
                          .map((cat, index) => (
                            <span
                              key={index}
                              className="text-sm  text-gray-700 p-2 rounded-lg font-semibold bg-pureWhite-light shadow-md"
                            >
                              {cat}
                            </span>
                          ))}
                ...
                <div>
                  ...
                  <div className="bg-pageBg-light p-4 my-2 rounded-lg shadow-sm">
                    <span className="p-2 font-semibold px-3 py-2 text-gray-700 border rounded-lg focus:outline-none bg-pureWhite-light">
                      {data.mediaType}
                    </span>
                    {/* <input
                    id="mediaType"
