  ...
  return (
    <div className="flex items-center justify-between bg-white px-6 py-4 shadow-md">
      <div className="flex items-center gap-5">
        ...
        <div className="hidden lg:flex items-center space-x-4">
          <ul className="flex items-center space-x-4 cursor-pointer">
            <li className="text-gray-700 hover:text-black transition duration-300 ease-in-out" onClick={() => router.push('/')}>Images</li>
            <li className="text-gray-700 hover:text-black transition duration-300 ease-in-out" onClick={() => router.push('/video')}>Video</li>
            <li className="text-gray-700 hover:text-black transition duration-300 ease-in-out" onClick={() => router.push('/audio')}>Audio</li>
          </ul>
        </div>
      </div>
      ...
      <div className="flex lg:hidden items-center space-x-4">
        ...
        <CartPopup />
        <AiOutlineMenu className="text-gray-700 w-6 h-6 cursor-pointer hover:text-black transition-transform duration-200 ease-in-out hover:scale-110" onClick={() => setMenuOpen(true)} />
      </div>

      {menuOpen && (
        <div className="fixed inset-0 bg-white z-50 flex flex-col p-4">
          <div className="flex justify-between items-center">
            <AiOutlineClose className="text-gray-700 w-6 h-6 cursor-pointer hover:text-black transition-transform duration-200 ease-in-out hover:scale-110" onClick={() => setMenuOpen(false)} />
          </div>
          <ul className="mt-4 space-y-3">
            <li className="block text-gray-700 hover:text-black py-2 transition-colors duration-300 ease-in-out" onClick={() => router.push('/')}>Images</li>
            <li className="block text-gray-700 hover:text-black py-2 transition-colors duration-300 ease-in-out" onClick={() => router.push('/video')}>Video</li>
            <li className="block text-gray-700 hover:text-black py-2 transition-colors duration-300 ease-in-out" onClick={() => router.push('/audio')}>Music</li>
          </ul>
        </div>
      )}
    </div>
